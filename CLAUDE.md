...
- `web_source.py` — Fetches `https://tldr.tech/<topic>/<YYYY-MM-DD>` for each requested topic, parses the HTML with BeautifulSoup, skips redirected URLs silently (no edition for that date), dedupes articles across topics, and filters sponsor/promo sections. Returns `list[Article]`. Both the availability probe (`check_availability()`) and the newsletter fetch (`fetch_newsletters()`) probe topics **sequentially with a randomised inter-request delay** (`web.check_delay_min`/`web.check_delay_max`, default 0.5–2.0 s) so tldr.tech does not rate-limit a 13-topic burst and return false `404`s. Set both bounds to 0 (or pass `delay_range=None`) to restore the old concurrent, no-delay behaviour.
- `web_scraper.py` — Fetches full article text via trafilatura; falls back to the newsletter summary if scraping fails. Populates `Article.full_text`. When the optional `cloakbrowser` package is installed and `scraping.cloak_fallback` is not `off`, articles that trafilatura cannot fetch or extract are re-rendered by a stealth Chromium browser (CloakBrowser, launched with `humanize=True`) before falling back to the summary. After navigation, the fallback waits up to 35 s for any Cloudflare Turnstile challenge to resolve. At most 2 concurrent browser sessions run at once. Known limitation: enterprise bot management (e.g. g2.com) may still block the fallback — it handles standard Cloudflare challenges only.
...
- `web.timeout_seconds` — HTTP timeout in seconds for newsletter page fetches (default: 15)
- `web.check_delay_min` / `web.check_delay_max` — bounds (seconds) of the randomised pause inserted between successive tldr.tech requests, for both the availability probe and the newsletter fetch (defaults: 0.5 / 2.0). Set both to 0 to disable and restore concurrent, no-delay probing.
