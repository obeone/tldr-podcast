"""
Link extractor and categoriser for TLDR newsletter articles.
...
def extract_links(articles: list[Article]) -> LinkReport:
    """
    Extract and categorise all links from a list of parsed articles.

    For each article, the primary ``article.url`` is added as a ``"source"``
    link.  Additional URLs found in ``article.full_text`` (or
    ``article.summary`` when full text is empty) are categorised and added
    ...
    for article in articles:
        ...
        body = article.full_text or article.summary or ""
        for url in extract_links_from_text(body):
            ...
            link = CategorisedLink(url=url, label=article.title, category=category)
